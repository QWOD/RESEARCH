...
gitHubKey=${homePath}/.ssh/id_ed25519
...
[[ "${fastForwardOnly}" == true ]] && repoPath=${homePath}/hyperstor/QWOD # :[[ :REMOTE-GIT-TREE: ]]:
[[ "${fastForwardOnly}" != true ]] && repoPath=${homePath}/QWOD
[[ $* == "start" || $* == "stop" || $* == "-r" ]] || echo -E '[[ "USAGE: requires: either: [[ start: || stop: || -r: ]]" ]]:' || exit 0

if [[ $* == "stop" ]]; then
  if [[ "${pushRepo}" == true || "${fastForwardOnly}" != true ]]; then
    if toolbox run ${homePath}/bin/gitupur push; then
...
if [[ $* == "start" || $* == "-r" ]]; then
  ...
  if [[ "${sshSystemDHook}" == true ]]; then
    ...
    toolbox run /usr/bin/env ssh "${sshHost}" -o "StrictHostKeyChecking no" -i "${gitHubKey}" -t "/usr/bin/env bash -c ${homePath}/hyperstor/bin/figtreeservice start " # &> /dev/null"
    ...
    while true; do
      if cd "${repoPath}/RESEARCH"; then
        [[ $fastForwardOnly == true ]] && toolbox run ${homePath}/bin/newfig -f
        [[ $pushRepo == true ]] &&  toolbox run ${homePath}/bin/newfig -p # &> /dev/null
      fi
    ...
    while true; do
      if cd "${repoPath}/RESEARCH"; then
        [[ $fastForwardOnly == true ]] && ${homePath}/bin/newfig -f
        [[ $pushRepo == true ]] &&  ${homePath}/bin/newfig -p # &> /dev/null
      fi
      if cd "${repoPath}/HYPERMEDIUS"; then
        [[ $fastForwardOnly == true ]] && ${homePath}/bin/newfig -f
        [[ $pushRepo == true ]] && ${homePath}/bin/newfig -p # &> /dev/null
