...
function cloudHiveBackup {
  ...
  [[ "push" == "${specialWarfareTask}" ]] && specialOperations=(
    "toolbox run /usr/bin/env ssh -o 'StrictHostKeyChecking no' -T git@github.com &> /dev/null; /usr/bin/env git config --global user.email ${GHUSER}; /usr/bin/env git config --global user.name ':QWOD-MJ12: ATSOSSDEV-A: SPG-OMEGA:'"
    "toolbox run cd ~/QWOD/HYPERMEDIUS && ~/bin/newfig -p; cd ~/QWOD/RESEARCH && ~/bin/newfig -p; cd ~/QWOD/DISCLOSURE && ~/bin/newfig -p"
  )
